...
def mafft(gene, path_in, path_out, done):
    ...
    options = {'cores': 1, 'memory': "250g", 'walltime': "06:00:00", 'account':"Coryphoideae"}
    ...
    mafft --localpair --adjustdirectionaccurately --thread 1 {gene}.FNA > {path_out}{gene}_aligned.fasta
...
def optrim_preb(path_in,done,gene,path_out):
    ...
    outputs = [path_out+"0.1/"+gene,path_out+"0.15/"+gene,path_out+"0.20/"+gene,path_out+"0.25/"+gene,path_out+"0.3/"+gene,
    path_out+"0.33/"+gene,path_out+"0.4/"+gene,path_out+"0.45/"+gene,path_out+"0.5/"+gene,path_out+"0.55/"+gene,path_out+"0.6/"+gene,
    path_out+"0.67/"+gene,path_out+"0.7/"+gene,path_out+"0.75/"+gene,path_out+"0.8/"+gene,path_out+"0.85/"+gene,path_out+"0.9/"+gene,path_out+"0.95/"+gene] 
...
def amas_preb_gt(path_in,path_out,done,cut_off,gene):
    """Creating summary files for all the trimmed alignments for each GT value"""
    inputs = [path_in+"0.1/"+gene,path_in+"0.15/"+gene,path_in+"0.20/"+gene,path_in+"0.25/"+gene,path_in+"0.3/"+gene,
    path_in+"0.33/"+gene,path_in+"0.4/"+gene,path_in+"0.45/"+gene,path_in+"0.5/"+gene,path_in+"0.55/"+gene,path_in+"0.6/"+gene,
    path_in+"0.67/"+gene,path_in+"0.7/"+gene,path_in+"0.75/"+gene,path_in+"0.8/"+gene,path_in+"0.85/"+gene,path_in+"0.9/"+gene,path_in+"0.95/"+gene]
...
def amas_preb_raw(path_in,path_out,done,gene):
...
# I need to write and amas script that calculates all the summaries for all the trimmed files and for all the raw files
# Also i need to find out if these summaries are needed for the optrimal R script.
# cd ${cutoff_trim}
...
gt_values =["0.1","0.15","0.20","0.25","0.3","0.33","0.4","0.45","0.5","0.55","0.6","0.67","0.7","0.75","0.8","0.85","0.9","0.95"]
...
for i in range(len(genes)):
    ...
    #### Running amas preb gt
    for j in range(len(gt_values)):
        gwf.target_from_template('amas_gt_'+gt_values[j], amas_preb_gt(gene = genes[i], cut_off=gt_values[j],
                                                        path_in = "/home/owrisberg/Coryphoideae/work_flow/07_mapping/",
                                                        path_out = "/home/owrisberg/Coryphoideae/work_flow/08_optrimal",
                                                        done = "/home/owrisberg/Coryphoideae/work_flow/08_optrimal/done/amas_gt/"+genes[i]))
    
    gwf.target_from_template('amas_raw', amas_preb_raw(gene = genes[i], cut_off=gt_values[j],
                                                        path_in = "/home/owrisberg/Coryphoideae/work_flow/07_mapping/",
                                                        path_out = "/home/owrisberg/Coryphoideae/work_flow/08_optrimal",
                                                        done = "/home/owrisberg/Coryphoideae/work_flow/08_optrimal/done/amas_gt/"+genes[i]))

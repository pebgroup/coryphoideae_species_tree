...
def retrieve(path_in, species, path_out,genes_list):
    """Retrieve gene sequences from all the species and create an unaligned multifasta for each gene."""
    inputs = ["/home/owrisberg/Coryphoideae/work_flow/04_coverage/done/"+species]
    outputs = [path_in + "outstats.csv", path_out+genes_list ]
    options = {'cores': 10, 'memory': "20g", 'walltime': "10:00:00", 'account':"Coryphoideae"}
...
def mafft(gene, path_in, path_out, done):
    ...
    linsi --adjustdirectionaccurately --thread 64 {gene}.FNA > {path_out}{gene}_aligned.fasta
